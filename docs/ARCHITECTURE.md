...
**X — Output token discipline (soft caps via system prompt).** The Claude CLI has no `--max-tokens` flag, so X is enforced through `--append-system-prompt` brevity hints. trivial: "Output only the answer." simple: "Be concise. Skip preamble." standard: "Aim for under 4000 tokens. Prefer bullets and code over prose." hard/reasoning/max: no hint (suppress constraint on complex tasks). `spec.maxOutputTokens` exists on the profile for telemetry budgeting only — it is not currently wired to a generation-time flag.
