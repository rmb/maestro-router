...
**Embedding** (optional peer `@huggingface/transformers` v3 or v4) computes cosine similarity between the prompt embedding and 76 frozen labeled exemplars across six classes. Returns null if the peer isn't installed — the LLM stage is the final fallback. The inference model is configurable via `embeddingModel` in `~/.maestro/config.json`; the model must match the one used to build the exemplars, and a mismatch is rejected at runtime with the `fallback.embedding_model_mismatch` diagnostic (bge/e5 families also get an automatic `query:` prefix at both embed-time and runtime). The confidence floor is the configurable `embeddingMinSimilarity` (default 0.4) — the minimum cosine similarity on the default path, or the minimum calibrated probability on the head path. Setting `embeddingHeadPath` to a SetFit logistic-head JSON replaces cosine-nearest-exemplar with calibrated per-class probabilities from the head (exemplars are then not consulted); the head's embedding dimension must match `embeddingModel`, else `fallback.embedding_head_dim_mismatch` fires.
